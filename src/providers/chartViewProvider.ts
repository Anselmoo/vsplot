...
export class ChartViewProvider implements vscode.WebviewViewProvider {
	...
	private _getHtmlForWebview(webview: vscode.Webview) {
        ...
        <head>
            ...
            <meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src ${webview.cspSource} data: blob:; style-src 'unsafe-inline' ${webview.cspSource}; script-src 'nonce-${nonce}' ${webview.cspSource}; font-src ${webview.cspSource}; connect-src ${webview.cspSource} data:;">
            ...
            <style>
                ...
                .chart-stats .section-title { font-weight: 600; color: var(--vscode-descriptionForeground); margin-bottom: 8px; }
                .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 8px 16px; font-variant-numeric: tabular-nums; }
                ...
                .chart-stats.compact { padding: 10px 12px; border-radius: 6px; }
                .chart-stats.compact .stats-grid { gap: 6px 12px; }
                .chart-stats.compact .badge { width:16px; height:16px; font-size:11px; }
        ...
        <body>
            ...
            <div class="controls">
                ...
                <div class="control-group" id="formatGroup">
                    ...
                    <label style="display:flex;align-items:center;gap:4px;">
                        <input type="checkbox" id="thousands" /> 1,000s
                    </label>
                </div>

                <div class="control-group">
                    <label for="compactCardsToggle">Cards:</label>
                    <input type="checkbox" id="compactCardsToggle" ${vscode.workspace.getConfiguration("vsplot").get("compactStatsCards", false) ? "checked" : ""} />
                    <span>Compact</span>
            ...
            <div class="chart-stats ${"${vsplotConfig.get<boolean>('compactStatsCards', false) ? " + "'compact'" + " : " + "''" + "}"}" id="chartStats" style="display: none;"></div>
            <div class="chart-stats ${"${vsplotConfig.get<boolean>('compactStatsCards', false) ? " + "'compact'" + " : " + "''" + "}"}" id="chartMeta" style="display: none;"></div>
            ...
            <script nonce="${nonce}">
                ...
                let compactCards = ${vsplotConfig.get<boolean>("compactStatsCards", false)};
                let showIcons = ${vsplotConfig.get<boolean>("showStatsIcons", true)};
