...
export class ChartViewProvider implements vscode.WebviewViewProvider {
	...
	public async applyChartConfig(config: ChartTestConfig): Promise<void> {
		...
		const compactCards = vsplotConfig.get<boolean>(
			"compactStatsCards",
			false,
		);
		...
		const cssUri = webview.asWebviewUri(
			vscode.Uri.joinPath(this._extensionUri, "media", "chartView.css"),
		);
		const scriptUri = webview.asWebviewUri(
			vscode.Uri.joinPath(this._extensionUri, "media", "chartView.js"),
		);
		...
		try {
			const htmlBuffer = require("fs").readFileSync(htmlPath.fsPath, "utf8");
			htmlContent = htmlBuffer;
		} catch (error) {
			// Fallback to minimal HTML if template file can't be loaded
			return `<!DOCTYPE html><html><body><h1>Error loading chart view</h1></body></html>`;
		}

		// Replace placeholders in HTML template
		const cspContent = `default-src 'none'; img-src ${webview.cspSource} data: blob:; style-src 'unsafe-inline' ${webview.cspSource}; script-src 'nonce-${nonce}' ${webview.cspSource}; font-src ${webview.cspSource}; connect-src ${webview.cspSource} data:;`;

		const compactCardsClass = compactCards ? "compact" : "";
		const compactCardsChecked = compactCards ? "checked" : "";

		// Chart type selections
		const lineSelected = defaultChartType === "line" ? "selected" : "";
		const barSelected = defaultChartType === "bar" ? "selected" : "";
		const scatterSelected = defaultChartType === "scatter" ? "selected" : "";
		const pieSelected = defaultChartType === "pie" ? "selected" : "";

		htmlContent = htmlContent
			.replace(/\{\{cspContent\}\}/g, cspContent)
			.replace(/\{\{nonce\}\}/g, nonce)
			.replace(/\{\{chartJsUri\}\}/g, chartJsUri.toString())
			.replace(/\{\{zoomPluginUri\}\}/g, zoomPluginUri.toString())
			.replace(/\{\{dateAdapterUri\}\}/g, dateAdapterUri.toString())
			.replace(/\{\{cssUri\}\}/g, cssUri.toString())
			.replace(/\{\{scriptUri\}\}/g, scriptUri.toString())
			.replace(/\{\{defaultStylePreset\}\}/g, defaultStylePreset)
			.replace(/\{\{defaultDecimals\}\}/g, defaultDecimals.toString())
			.replace(
				/\{\{defaultUseThousands\}\}/g,
				defaultUseThousands.toString(),
			)
			.replace(/\{\{compactCards\}\}/g, compactCards.toString())
			.replace(/\{\{showIcons\}\}/g, showIcons.toString())
			.replace(/\{\{compactCardsClass\}\}/g, compactCardsClass)
			.replace(/\{\{compactCardsChecked\}\}/g, compactCardsChecked)
			.replace(/\{\{lineSelected\}\}/g, lineSelected)
			.replace(/\{\{barSelected\}\}/g, barSelected)
			.replace(/\{\{scatterSelected\}\}/g, scatterSelected)
			.replace(/\{\{pieSelected\}\}/g, pieSelected);
