...
suite("Delimiter Detection Tests", () => {
	test("Detects colon delimiter", async function () {
		this.timeout(10000);
		const uri = vscode.Uri.file(path.join(__dirname, "../../test-data/colon-delimited.txt"));
	...
	test("Detects pipe delimiter", async function () {
		this.timeout(10000);
		const uri = vscode.Uri.file(path.join(__dirname, "../../test-data/pipe-delimited.dat"));
		const data = await parseDataFile(uri);
	...
	test("Detects space delimiter", async function () {
		this.timeout(10000);
		const uri = vscode.Uri.file(path.join(__dirname, "../../test-data/space-delimited.txt"));
		const data = await parseDataFile(uri);
	...
	test("User can override delimiter with pipe", async function () {
		...
		assert.strictEqual(dataAuto?.detectedDelimiter, ",", "Should auto-detect comma");
		...
		assert.strictEqual(dataOverride?.detectedDelimiter, "|", "Should use override delimiter");
	...
	test("User can override delimiter with tab", async function () {
		...
		assert.ok(data, "Data should be parsed");
		assert.strictEqual(data?.detectedDelimiter, "\t", "Should use tab delimiter");
		assert.strictEqual(data?.headers.length, 3, "Should have 3 columns");
	...
	test("Delimiter detection prefers consistency across lines", async function () {
		...
		const content = "a,b,c\n1,2,3\n4,5,6\n7,8,9";
		const tmpPath = path.join(__dirname, "../../test-data/consistency-test.txt");
		await vscode.workspace.fs.writeFile(vscode.Uri.file(tmpPath), Buffer.from(content, "utf8"));
	...
	test("Delimiter detection handles semicolon", async function () {
		...
		assert.ok(data, "Data should be parsed");
		assert.strictEqual(data?.detectedDelimiter, ";", "Should detect semicolon delimiter");
		assert.strictEqual(data?.headers.length, 3, "Should have 3 columns");
		assert.strictEqual(data?.rows.length, 2, "Should have 2 data rows");
