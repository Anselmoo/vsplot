...
suite("Message Handler Unit Tests", () => {
	...
	suite("toCSV", () => {
		...
		test("should handle null and undefined values", () => {
			const result = toCSV(
				["A", "B"],
				[[null as unknown as string, undefined as unknown as number]],
			);
	...
	suite("handleReparse", () => {
		...
		test("should post message with parsed data", async () => {
			...
			await handleReparse(
				...
				vscode.Uri.file("/test.csv"),
				async (msg) => {
					postedMessage = msg as { type: string; data: ParsedData };
					return true;
				},
				deps,
			);
		...
		test("should use correct URI for parsing", async () => {
			...
			await handleReparse(message, testUri, async () => true, deps);
