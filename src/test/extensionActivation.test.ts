...
suite("Extension Activation Tests", () => {
	...
	test("All commands should be registered", async function () {
		...
		assert.ok(commands.includes("vsplot.openDataViewer"), "openDataViewer command should be registered");
		
		// Check test commands
		assert.ok(commands.includes("vsplot.test.applyChartConfig"), "test.applyChartConfig command should be registered");
		assert.ok(commands.includes("vsplot.test.requestChartState"), "test.requestChartState command should be registered");
	});
