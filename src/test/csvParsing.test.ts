...
suite("CSV Parsing Edge Cases Tests", () => {
	...
	test("CSV with quoted fields containing newlines should handle gracefully", async function () {
		...
		assert.strictEqual(data?.rows.length, 2, "Should have 2 data rows");
		assert.strictEqual(data?.rows[0][1], "Simple note", "Quoted field should be parsed");
		assert.strictEqual(data?.rows[1][1], "Another note", "Quoted field should be parsed");
	...
	test("CSV with empty quoted fields should parse", async function () {
		...
		assert.strictEqual(data?.headers.length, 3, "Should have 3 columns");
		assert.strictEqual(data?.rows.length, 2, "Should have 2 data rows");
		assert.strictEqual(data?.rows[0][1], "", "Empty quoted field should be empty string");
		assert.strictEqual(data?.rows[1][1], "", "Empty quoted field should be empty string");
	...
	test("CSV with all numeric first line should generate column headers", async function () {
		...
		assert.ok(data, "Data should be parsed");
		assert.strictEqual(data?.headers.length, 3, "Should have 3 columns");
		assert.strictEqual(data?.headers[0], "Column 1", "Should auto-generate header");
		assert.strictEqual(data?.headers[1], "Column 2", "Should auto-generate header");
		assert.strictEqual(data?.headers[2], "Column 3", "Should auto-generate header");
	...
	test("CSV with mixed numeric and text headers should detect headers", async function () {
		...
		assert.strictEqual(data?.headers[1], "Name", "Second header should be 'Name'");
		assert.strictEqual(data?.rows.length, 2, "Should have 2 data rows");
	...
	test("CSV with leading and trailing empty lines should be filtered", async function () {
		...
		assert.ok(data, "Data should be parsed");
		assert.strictEqual(data?.headers.length, 2, "Should have 2 columns");
		assert.strictEqual(data?.rows.length, 2, "Should have 2 data rows (empty lines filtered)");
	...
	test("CSV with spaces around values should trim them", async function () {
		...
		assert.strictEqual(data?.rows[0][1], 25, "Numeric values should be parsed and trimmed");
