...
**Important:** Before running tests locally, you must download the test data:

```bash
bash scripts/setup-test-data.sh
```

This downloads sample datasets and test fixtures to avoid committing third-party data with potential license issues. The CI pipeline runs this automatically.
...
Test fixtures are downloaded by `scripts/setup-test-data.sh` and stored in:
- `sample-data/` - Sample datasets for manual testing and demos
- `test-data/` - Test fixtures for automated tests

Both directories are gitignored to avoid committing third-party data.

**Why download instead of commit?**
- Avoids potential license issues with test datasets
- Keeps repository size small
- Ensures fresh data from original sources
