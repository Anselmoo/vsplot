 ...
 */
	...
	var formatters$2 = {
		...
		// Hour [1-12]
		h: (date, token) => addLeadingZeros(date.getUTCHours() % 12 || 12, token.length),
		// Hour [0-23]
		...
		S: (date, token) => {
			...
			var fractionalSeconds = Math.floor(milliseconds * 10 ** (numberOfDigits - 3));
	...
	function getUTCWeek(dirtyDate, options) {
		...
		var date = toDate(dirtyDate);
		var diff =
			startOfUTCWeek(date, options).getTime() - startOfUTCWeekYear(date, options).getTime(); // Round the number of days to the nearest integer
		// because the number of milliseconds in a week is not constant
		// (e.g. it's different in the week of the daylight saving time clock shift)
	...
	function assign(target, dirtyObject) {
		...
		for (var property in dirtyObject) {
			if (Object.hasOwn(dirtyObject, property)) {
	...
	var parsers = {
		...
		y: {
			...
			validate: (_date, value, _options) => value.isTwoDigitYear || value.year > 0,
			set: (date, flags, value, _options) => {
		...
		S: {
			priority: 30,
			parse: (string, token, _match, _options) => {
				var valueCallback = (value) => Math.floor(value * 10 ** (-token.length + 3));
