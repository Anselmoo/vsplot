		...
		case "vsplot:test:getState":
			try {
				const state = {
					...
					dragZoom: document.getElementById("dragZoomToggle")?.checked ?? true,
					...
					logX: document.getElementById("logXToggle")?.checked ?? false,
					logY: document.getElementById("logYToggle")?.checked ?? false,
					labelsCount:
...
function initializeChart() {
	...
	if (saved) {
		try {
			...
			if (typeof saved.scientific === "boolean") {
				scientific = saved.scientific;
			}
			applyFormatToUI(decimals, scientific);
			const logXToggle = document.getElementById("logXToggle");
			if (logXToggle && typeof saved.logX === "boolean") logXToggle.checked = saved.logX;
			const logYToggle = document.getElementById("logYToggle");
			if (logYToggle && typeof saved.logY === "boolean") logYToggle.checked = saved.logY;
...
function createChart() {
	try {
		...
		const logState = syncLogState(!isNaN(yAxis2Index) && yAxis2Index >= 0 && !isCategoricalY2);
		...
		if (logState.x && (isCategoricalX || xIsTime)) {
			showError("Log X requires numeric, non-time data.");
			return;
		}

		if (logState.y && isCategoricalY) {
			showError("Log Y requires numeric data.");
			return;
		}

		const yValuesForLog = currentData.rows
			.map((row) => parseFloat(row[yAxisIndex]))
			.filter((v) => Number.isFinite(v));
		const y2ValuesForLog =
			!isNaN(yAxis2Index) && yAxis2Index >= 0
				? currentData.rows
						.map((row) => parseFloat(row[yAxis2Index]))
						.filter((v) => Number.isFinite(v))
				: [];
		const xValuesForLog =
			!isCategoricalX && !xIsTime
				? currentData.rows
						.map((row) => parseFloat(row[xAxisIndex]))
						.filter((v) => Number.isFinite(v))
				: [];

		if (logState.x && hasNonPositive(xValuesForLog)) {
			showError("Log X requires all positive values.");
			return;
		}
		if (logState.y && (hasNonPositive(yValuesForLog) || hasNonPositive(y2ValuesForLog))) {
			showError("Log Y requires all positive values.");
			return;
		}
		...
		const dragEnabled = document.getElementById("dragZoomToggle")?.checked ?? true;
		...
		try {
			...
			byFile[currentData.fileName] = {
				...
				legend: document.getElementById("legendToggle")?.checked ?? true,
				dragZoom: document.getElementById("dragZoomToggle")?.checked ?? true,
				curveSmoothing:
					chartType === "line"
						? document.getElementById("curveToggle").checked
				...
				scientific,
				logX: logScaleState.x,
				logY: logScaleState.y,
			};
...
function getChartOptions(
	...
	if (
		...
		const xScaleType = logState?.x ? "logarithmic" : detectXScaleType();
		baseOptions.scales = {
			x: {
				type: xScaleType,
				ticks: {
			...
			y: {
				type: logState?.y ? "logarithmic" : undefined,
				ticks: {
					color: fg,
					callback: (v) => {
						try {
		...
		if (hasY2) {
			baseOptions.scales.y2 = {
				position: "right",
				type: logState?.y2 ? "logarithmic" : undefined,
				ticks: {
					color: fg,
					callback: (v) => {
						try {
...
function detectXScaleType() {
	if (logScaleState?.x) return "logarithmic";
 ...
 */
function getPalette() {
	if (stylePreset === "soft" || stylePreset === "vibrant")
		return [
			"rgba(99, 110, 250, 0.6)",
			"rgba(239, 85, 59, 0.6)",
			"rgba(0, 204, 150, 0.6)",
		...
		stats.innerHTML = `
            ...
            <div class="stats-grid">
                ...
                <div class="stat">
					...
					<div><strong>Min:</strong> ${formatNumber(min)}</div>
                </div>
                <div class="stat">
                    <span class="badge">
					...
					<div><strong>Max:</strong> ${formatNumber(max)}</div>
                </div>
                <div class="stat">
                    <span class="badge">
					...
					<div><strong>Average:</strong> ${formatNumber(avg)}</div>
                </div>
                <div class="stat">
                    <span class="badge">
					...
					<div><strong>Median:</strong> ${formatNumber(median)}</div>
                </div>
                <div class="stat">
                    <span class="badge">
					...
					<div><strong>Std Dev:</strong> ${formatNumber(stddev)}</div>
                </div>
            </div>
        `;
...
>>>>>>> 981502c (Remove non-functional pan toggle & document arrow-key pan navigation)
...
// Keep thousands/change listeners and arrow-key pan navigation
...
const logXToggleControl = document.getElementById("logXToggle");
if (logXToggleControl) logXToggleControl.addEventListener("change", createChart);
const logYToggleControl = document.getElementById("logYToggle");
if (logYToggleControl) logYToggleControl.addEventListener("change", createChart);
...
function applyFormatToUI(dec = defaultDecimals, sci = false) {
	const decSel = document.getElementById("decimals");
	if (!decSel) return;
	scientific = !!sci;
	if (scientific) {
		decimals = typeof dec === "number" ? dec : defaultDecimals;
		decSel.value = "sci";
		return;
	}
	const value = typeof dec === "number" ? String(dec) : String(defaultDecimals);
	decimals = parseInt(value) || defaultDecimals;
	decSel.value = value;
}

function syncFormatFromUI() {
	const decSel = document.getElementById("decimals");
	const val = decSel ? decSel.value : String(defaultDecimals);
	scientific = val === "sci";
	if (scientific) {
		decimals = Number.isFinite(decimals) ? decimals : defaultDecimals;
	} else {
		decimals = parseInt(val) || defaultDecimals;
	}
}

function syncLogState(hasY2Candidate = false) {
	const logX = document.getElementById("logXToggle")?.checked ?? false;
	const logY = document.getElementById("logYToggle")?.checked ?? false;
	return {
		x: logX,
		y: logY,
		y2: logY && !!hasY2Candidate,
	};
}
