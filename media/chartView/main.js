    ...
    if (chartType === 'pie' || chartType === 'doughnut') {
        ...
        if (chartType === 'scatter') {
            ...
            if (chartType === 'line' && xIsTimeLocal) {
                ...
                const points2 = hasY2 ? currentData.rows.map(row => ({ 
                    x: new Date(row[xAxisIndex]), 
                    y: parseFloat(row[yAxis2Index]) || 0 
                })) : undefined;
                result = {
...
function getPalette() {
    if (stylePreset === 'vibrant') return [
        ...
        'rgba(0,200,83,0.85)','rgba(233,30,99,0.85)'
    ];
    if (stylePreset === 'soft') return [
        'rgba(99, 110, 250, 0.6)','rgba(239, 85, 59, 0.6)','rgba(0, 204, 150, 0.6)',
        'rgba(171, 99, 250, 0.6)','rgba(255, 161, 90, 0.6)','rgba(25, 211, 243, 0.6)',
        'rgba(255, 102, 146, 0.6)'
    ];
    return [
        'rgba(120, 144, 156, 0.7)','rgba(33, 150, 243, 0.7)','rgba(156, 39, 176, 0.7)',
        'rgba(76, 175, 80, 0.7)','rgba(255, 193, 7, 0.7)','rgba(244, 67, 54, 0.7)',
        'rgba(0, 188, 212, 0.7)'
    ];
}
