...
((global, factory) => {
	typeof exports === "object" && typeof module !== "undefined"
		...
		: typeof define === "function" && define.amd
			...
			: ((global = typeof globalThis !== "undefined" ? globalThis : global || self),
				(global.ChartZoom = factory(global.Chart, global.Hammer, global.Chart.helpers)));
})(this, (chart_js, Hammer, helpers) => {
	...
	function getEnabledScalesByPoint(options, point, chart) {
		...
		const enabledScales = [];
		helpers.each(chart.scales, (scaleItem) => {
	...
	function logarithmicZoomRange(scale, zoom, center) {
		...
		return {
			min: 10 ** (logMin + delta.min),
			max: 10 ** (logMax - delta.max),
		};
	}
	...
	function zoom(chart, amount, transition = "none", trigger = "api") {
		...
		helpers.each(enabledScales || chart.scales, (scale) => {
	...
	var Zoom = {
		...
		start: (chart, _args, options) => {
			...
			if (Object.hasOwn(options.zoom, "enabled")) {
				console.warn(
					"The option `zoom.enabled` is no longer supported. Please use `zoom.wheel.enabled`, `zoom.drag.enabled`, or `zoom.pinch.enabled`.",
				);
			}
			if (
				Object.hasOwn(options.zoom, "overScaleMode") ||
				Object.hasOwn(options.pan, "overScaleMode")
			) {
				console.warn(
